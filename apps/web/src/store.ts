...
function normalizeThreadFromReadModel(
  ...
  const pendingSourceProposedPlan = latestTurn?.sourceProposedPlan;
  ...
  if (
    ...
    previous.latestUserMessageAt === incoming.latestUserMessageAt &&
    previous.hasPendingApprovals === incoming.hasPendingApprovals &&
    previous.hasPendingUserInput === incoming.hasPendingUserInput &&
    previous.hasActionableProposedPlan === incoming.hasActionableProposedPlan &&
  ...
  return {
    ...
    hasPendingApprovals: incoming.hasPendingApprovals ?? false,
    hasPendingUserInput: incoming.hasPendingUserInput ?? false,
    hasActionableProposedPlan: incoming.hasActionableProposedPlan ?? false,
    lastVisitedAt,
...
// Preserve the proposed-plan source across the live turn lifecycle until the read model catches up.
...
function buildSidebarThreadSummary(
  ...
  const nextSummary: SidebarThreadSummary = {
    ...
    latestUserMessageAt: thread.latestUserMessageAt ?? deriveLatestUserMessageAt(thread.messages),
    hasPendingApprovals: thread.hasPendingApprovals ?? hasPendingApprovalsSignal(thread.activities),
    hasPendingUserInput: thread.hasPendingUserInput ?? hasPendingUserInputSignal(thread.activities),
    hasActionableProposedPlan:
      thread.hasActionableProposedPlan ??
      hasActionableProposedPlanSignal(thread.proposedPlans, thread.latestTurn),
...
> {
  return {
    ...
    hasPendingApprovals: hasPendingApprovalsSignal(thread.activities),
    hasPendingUserInput: hasPendingUserInputSignal(thread.activities),
...
function applyThreadMessageSentEvent(thread: Thread, event: ThreadMessageSentEvent): Thread {
  ...
  if (
    ...
    const nextRequestedAt = previousTurn?.requestedAt ?? payload.createdAt;
    const nextStartedAt = previousTurn?.startedAt ?? payload.createdAt;
    const nextCompletedAt = payload.streaming
    ...
    latestTurn =
      previousTurn &&
      previousTurn.turnId === nextLatestTurn.turnId &&
      previousTurn.state === nextLatestTurn.state &&
      previousTurn.requestedAt === nextLatestTurn.requestedAt &&
      previousTurn.startedAt === nextLatestTurn.startedAt &&
      previousTurn.completedAt === nextLatestTurn.completedAt &&
      previousTurn.assistantMessageId === nextLatestTurn.assistantMessageId &&
      previousTurn.sourceProposedPlan === nextLatestTurn.sourceProposedPlan
        ? previousTurn
        : nextLatestTurn;
  }
